...
# while .bashrc is executed for an interactive non-login shell.
...
#######################################################
# The below are for .bash_profile
#######################################################

if [[ -n $SSH_CONNECTION ]] ; then
    echo "I am logged in remotely in HOST:("$HOSTNAME")"
else
    echo "This is local connection in HOST:("$HOSTNAME")"
fi



#######################################################
# Update terminal title after leaving host SSH
##############################
function title()
{
...
function ssh()
{
   ...
   GREEN="\033[0;32m"
   title "Not in host:  "
   echo -e ${GREEN}"Leaving "${RED}$HOSTNAME${NC}" ... to "${RED}$@${NC}
   /usr/bin/ssh "$@"
}
...
function whatsmyip ()
{
	...
	echo -n "Internal IP: " ; /sbin/ifconfig eth0 | grep "inet" | awk -F '{print $1}'
	# External IP Lookup
	#echo -n "External IP: " ; wget http://smart-ip.net/myip -O - -q
    echo -n "External IP: " ; wget -qO - icanhazip.com
}
